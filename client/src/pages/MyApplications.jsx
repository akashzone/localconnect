...
function ApplicantCard({ app, onStatusUpdate }) {
  ...
  return (
    <div className="bg-white border border-[#D8D2C4] rounded-[6px] p-6 shadow-[3px_3px_0px_#D8D2C4]">
      ...
      {/* Developer identity */}
      <div className="space-y-1.5 pb-5 border-b border-dashed border-[#D8D2C4]">
        <p className="font-['Space_Grotesk'] font-bold text-[16px] flex items-center gap-2">
          <span aria-hidden="true">👤</span> {developer.name || "Unknown developer"}
        </p>
        <p className="font-['IBM_Plex_Mono'] text-[12.5px] text-[#4A473F] flex items-center gap-2">
          <span aria-hidden="true">📧</span> {developer.email || "—"}
        </p>
        {developer.title && (
          <p className="text-[13.5px] text-[#4A473F] flex items-center gap-2">
            <span aria-hidden="true">💻</span> {developer.title}
          </p>
        )}
      </div>

      {/* Cover letter / duration / status */}
      <div className="space-y-4 py-5 border-b border-dashed border-[#D8D2C4]">
        {app.coverLetter && (
          <div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-[14.5px] font-medium">{app.estimatedDuration || "—"}</p>
          </div>
