...
function EditProjectForm({ initialData, onSubmit, submitting, errorMsg, cancelTo }) {
    ...
    // Pre-fill once the parent has fetched the project
    useEffect(() => {
        if (initialData) {
            setFormData({
            ...
            setSkillsRequired(initialData.skillsRequired || initialData.skills || []);
        }
    }, [initialData]);
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            onChange={handleChange}
                            className="w-full border border-[#D8D2C4] rounded-[4px] px-3.5 py-2.5 text-[15px] bg-white
                         focus:outline-none focus:border-[#0F6B5C] focus:ring-2 focus:ring-[#0F6B5C]/15
                         transition-colors"
                        >
                            {statusOptions.map((s) => (
